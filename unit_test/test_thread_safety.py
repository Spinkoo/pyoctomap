...
class ThreadSafetyTestSuite:
            ...
            try:
                for _ in range(50):
                    ...
                    if resolution <= 0:
                        with lock:
                            consistency_errors.append(
                                f"Thread {thread_id}: Invalid resolution {resolution}"
                            )

                    if tree_depth < 0:
                        with lock:
                            consistency_errors.append(
                                f"Thread {thread_id}: Invalid tree depth {tree_depth}"
                            )

                    if num_nodes < 0:
                        with lock:
                            consistency_errors.append(
                                f"Thread {thread_id}: Invalid node count {num_nodes}"
                            )
                    ...
                    if (node1 is None) != (node2 is None):
                        with lock:
                            consistency_errors.append(
                                f"Thread {thread_id}: Inconsistent search results"
                            )
    ...
    def test_memory_safety_and_race_conditions(self):
        ...
        assert len(memory_errors) == 0, (
            f"Memory safety issues detected: {memory_errors}"
        )
    ...
    def test_performance_under_concurrent_load(self):
        ...
        if operations_per_second:
            ...
            assert avg_ops_per_sec > 10, (
                f"Performance too low: {avg_ops_per_sec} ops/sec"
            )
            assert total_ops_per_sec > 50, (
                f"Total performance too low: {total_ops_per_sec} ops/sec"
            )
            ...
            try:
                for _ in range(20):
                    ...
                    try:
                        ...
                        tree.addPointCloudWithRayCasting(
                            empty_points, np.array([0, 0, 0], dtype=np.float64)
                        )
                    except Exception as e:
                        with lock:
                    ...
                    try:
                        ...
                        tree.addPointCloudWithRayCasting(
                            nan_points, np.array([0, 0, 0], dtype=np.float64)
                        )
                    except Exception as e:
                        with lock:
                    ...
                    try:
                        ...
                        tree.addPointCloudWithRayCasting(
                            large_points, np.array([0, 0, 0], dtype=np.float64)
                        )
                    except Exception as e:
                        with lock:
                    ...
                    try:
                        ...
                        tree.addPointCloudWithRayCasting(
                            same_point.reshape(1, -1), same_point
                        )
                    except Exception as e:
                        with lock:
...
def test_basic_thread_safety():
    ...
    np.random.uniform(-5, 5, (100, 3)).astype(np.float64)
    sensor_origin = np.array([0.0, 0.0, 0.0], dtype=np.float64)
