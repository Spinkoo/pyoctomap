...
class CriticalThreadSafetyTest:
            ...
            try:
                for _ in range(self.test_iterations):
                    ...
                    if random.random() < 0.1:  # 10% chance
                        tree.clear()
                        new_points = np.random.uniform(-5, 5, (30, 3)).astype(
                            np.float64
                        )
                    ...
                    try:
                        ...
                        with lock:
                            dangling_errors.append(
                                f"Dangling iterator {thread_id}: {e}"
                            )
            ...
            try:
                for _ in range(50):
                    ...
                    if random.random() < 0.05:  # 5% chance
                        tree.clear()
                        new_points = np.random.uniform(-5, 5, (20, 3)).astype(
                            np.float64
                        )
                        tree.addPointCloudWithRayCasting(new_points, sensor_origin)
            ...
            try:
                for _ in range(30):
                    ...
                    try:
                        ...
                        with lock:
                            destruction_errors.append(
                                f"Post-destruction {thread_id}: {e}"
                            )
    ...
    def test_memory_corruption_in_shared_structures(self):
        ...
        assert len(corruption_errors) == 0, (
            f"Memory corruption detected: {corruption_errors}"
        )
                ...
                for _ in range(50):
                    ...
                    if node is not None:
                        try:
                            ...
                            with lock:
                                race_errors.append(
                                    f"Consistency check {thread_id}: {e}"
                                )
