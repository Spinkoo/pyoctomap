...
class ThreadSafetyStressTest:
    ...
    def test_iterator_invalidation_stress(self):
            ...
            try:
                for _ in range(50):
                    ...
                    if random.random() < 0.1:  # 10% chance
                        tree.clear()
                        new_points = np.random.uniform(-5, 5, (50, 3)).astype(np.float64)
        ...
        print(f"   ✅ Iterator invalidation stress test completed")
    ...
    def test_pointer_invalidation_stress(self):
        ...
        assert len(pointer_errors) == 0, f"Pointer invalidation detected: {pointer_errors}"
    ...
    def test_extreme_concurrency_scenarios(self):
        ...
        print(f"   ✅ Extreme concurrency test completed (errors: {len(extreme_errors)})")
