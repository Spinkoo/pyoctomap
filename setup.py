...
class CustomBuildExt(build_ext):
    ...
    def copy_libraries(self):
        """Copy shared libraries to package directory and create versioned symlinks"""
        ...
        for lib_file in lib_files:
            if os.path.exists(lib_file):
                ...
                if lib_file.endswith(".so"):
                    ...
                    elif "libdynamicedt3d.so" in lib_name and not lib_name.endswith(
                        ".1.10.0"
                    ):
                        versioned_names = [
                            "libdynamicedt3d.so.1.10",
                            "libdynamicedt3d.so.1.10.0",
                        ]
                    elif "liboctomath.so" in lib_name and not lib_name.endswith(
                        ".1.10.0"
                    ):
                        versioned_names = [
                            "liboctomath.so.1.10",
